                ...
                if(!$fb){
                   $_SESSION['error'] = "Ne mogu sacuvati plugin!";
                    ...
                    if (ftp_put($ftp, $remote_file, $folder, FTP_BINARY)) {
                        ...
                        $greska = "Dogodila se greÅ¡ka prilikom spremanja plugina.";
                        header("Location: gp-plugins.php?id=".$server_id);
                        die();
                   	}
                    unlink($folder);                                
                ...
                if(!$fw){
                    ...
                    if(!$fb) {
                        ...
                        if (ftp_put($ftp, $remote_file, $folder, FTP_BINARY)) {
                            ...
                            $_SESSION['error'] = "Dogodila se greÅ¡ka prilikom dodavanja admina.";
                            header("Location: gp-admins.php?id=".$server_id);
                            die();
                        }
                        unlink($folder);                                
...
if (isset($_GET['task']) && $_GET['task'] == "boost_server") {
	...
	$isticequery = $mdb->query("SELECT * FROM `t2` WHERE `ipport`= '$fullip'");
    $broj = mysqli_num_rows($isticequery);
	
	if($pp_server['igra'] != "1") {
	$_SESSION['info'] = "GREŠKA! - VAŠ SERVER NIJE CS 1.6";
	header("Location: $_SERVER[HTTP_REFERER]");
	die();
	}
	...
	if($broj == 0)
	{
	$querystr = "INSERT INTO `t2` (`ipport`, `type`, `vreme`) VALUES ('$fullip', 'cs', '$vreme')";
	if($mdb->query($querystr)=== TRUE)
	{
	$_SESSION['info'] = "Uspešno ste boostovali vaš server!";
	header("Location: $_SERVER[HTTP_REFERER]");
	die();
	}
	else
	{
	$_SESSION['info'] = "GREŠKA!";
	header("Location: $_SERVER[HTTP_REFERER]");
	die();
	}
	}
