...
function ssh_dodaj_server($ip, $port, $username, $password, $novi_user, $novi_user_pw, $mod)
{
	...
	else 
	{
		...
		else 
		{
			...
			$cmd_unzip = 'unzip '.$mod["zipname"].' && chown -Rf '.$novi_user.':'.$novi_user.' /home/'.$novi_user.'';
			$cmd_clean = 'rm '.$mod["zipname"].' && rm -rf wget-log && exit';
...
function reinstall_server($ip, $port, $username, $password, $serverid, $klijentid)
{
	...
	else 
	{
		...
		else 
		{
			...
			$cmd_unzip = 'unzip '.$mod["zipname"].' && chown -Rf '.$server["username"].':'.$server["username"].' /home/'.$server["username"].'';
			$cmd_clean = 'rm '.$mod["zipname"].' && rm -rf wget-log && exit';
